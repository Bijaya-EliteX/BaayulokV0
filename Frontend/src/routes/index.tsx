...
function TipsAndResources() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-20 md:px-6">
      <div className="text-center">
        <p className="text-sm font-semibold uppercase tracking-widest text-primary">Best Practices</p>
        <h2 className="mt-2 font-display text-4xl font-bold md:text-5xl">Tips & resources</h2>
        <p className="mx-auto mt-3 max-w-2xl text-lg text-muted-foreground">Six things every successful BaayuLok campaign does right.</p>
      </div>
      <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {tips.map((t, i) => (
          <motion.div 
            key={t.t} 
            className="rounded-2xl border border-border bg-card p-6 transition-all hover:-translate-y-1 hover:border-primary hover:shadow-lg"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
