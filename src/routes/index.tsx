...
import { ArrowRight, ShieldCheck, FileText, HandCoins, Play } from "lucide-react";
import { Button } from "@/components/ui/button";
import { CampaignCard } from "@/components/site/campaign-card";
import { campaigns, categories, stats, alliances, formatNpr } from "@/lib/mock-data";
...
function Stats() {
  const items = [
    { label: "Active Campaigns", value: stats.campaigns, suffix: "+" },
    { label: "Helping Hands", value: stats.donors, suffix: "+" },
    { label: "Raised for Nepal", value: stats.raised, prefix: "NPR ", format: true },
  ];
  return (
      ...
      <div className="mt-12 grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-7">
        {categories.map((c) => (
          <Link
            key={c.name}
            to="/categories"
            className="group rounded-2xl border border-border/60 bg-card p-6 text-center transition hover:-translate-y-1 hover:border-primary hover:shadow-lg"
          >
            <div className="text-4xl">{c.emoji}</div>
            <p className="mt-3 font-display text-sm font-semibold text-foreground">{c.name}</p>
            <p className="mt-1 text-xs text-muted-foreground">{c.count} campaigns</p>
          </Link>
        ))}
      </div>
    </section>
...
function HowItWorks() {
  const steps = [
    { n: 1, icon: FileText, t: "Submit your story", d: "Tell us who you're raising for and upload supporting documents." },
    { n: 2, icon: ShieldCheck, t: "Get KYC verified", d: "Our team verifies hospital letters, ID and beneficiary details within 48 hours." },
    { n: 3, icon: HandCoins, t: "Receive funds", d: "Donations land in your linked eSewa or bank account — every rupee tracked." },
  ];
  return (
    <section className="bg-[color:var(--cream)] py-20">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className="text-center">
          ...
          <h2 className="mt-2 font-display text-4xl font-bold md:text-5xl">Three steps, full transparency</h2>
        </div>
        <div className="mt-14 grid gap-6 md:grid-cols-3">
          {steps.map((s, i) => (
            ...
            >
              <span className="absolute -top-5 left-8 grid h-10 w-10 place-items-center rounded-full bg-primary font-display text-lg font-bold text-primary-foreground">{s.n}</span>
        ...
        <div className="relative grid place-items-center">
          <div className="aspect-[9/16] w-56 rounded-[2.5rem] border-4 border-background/20 bg-gradient-to-br from-primary via-primary to-[color:var(--saffron)] p-3 shadow-2xl">
            <div className="flex h-full flex-col rounded-[2rem] bg-background/95 p-4 text-foreground">
              ...
              <p className="font-display text-2xl font-bold text-primary">{formatNpr(stats.raised).slice(0, 12)}…</p>
              <div className="mt-3 space-y-2">
                {campaigns.slice(0, 2).map((c) => (
                  <div key={c.slug} className="rounded-xl bg-secondary p-2">
                    <div className="h-12 w-full overflow-hidden rounded"><img src={c.cover} alt="" className="h-full w-full object-cover" /></div>
